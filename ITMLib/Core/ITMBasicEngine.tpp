...
template <typename TVoxel, typename TIndex>
ITMTrackingState::TrackingResult ITMBasicEngine<TVoxel, TIndex>::ProcessFrame(ORUChar4Image *rgbImage, ORShortImage *rawDepthImage, ITMIMUMeasurement *imuMeasurement, ORUtils::SE3Pose *pose)
{
	// prepare image and turn it into a depth image
	...
	trackingState->supplied_pose->SetFrom(pose);
	trackingState->supplied_pose->Coerce();
	if (trackingActive)
		trackingController->Track(trackingState, view);
	...
	if (trackingState->age_pointCloud >= 0)
		...
		trackingState->framesProcessed = 0;
	...
	if (settings->behaviourOnFailure == ITMLibSettings::FAILUREMODE_RELOCALISE)
	{
		if (trackerResult == ITMTrackingState::TRACKING_GOOD && relocalisationCount > 0)
			relocalisationCount--;
		...
		//find and add keyframe, if necessary
		bool hasAddedKeyframe = relocaliser->ProcessFrame(view->depth, pose, 0, 1, &NN, &distances, trackerResult == ITMTrackingState::TRACKING_GOOD && relocalisationCount == 0);
